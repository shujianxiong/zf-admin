...
<body>
	...
	<div class="content-wrapper sub-content-wrapper">
		<section class="content-header content-header-menu">
			<h1>
				<small class="menu-active"><i class="fa fa-repeat"></i><a href="${ctx}/lgt/dp/dispatchOrder/callinList">执行调货入库任务列表</a></small>
			</h1>
		</section>		
		...
		<section class="content">
			<div class="box box-info">
			    ...
			    <form:form id="searchForm" modelAttribute="dispatchOrder" action="${ctx}/lgt/dp/dispatchOrder/callinList" method="post" class="form-horizontal">
								...
								<shiro:hasPermission name="lgt:dp:dispatchOrder:edit">
									<td>
										<div class="btn-group">
						                  ...
						                  <button id="selectQCUser" type="button" class="btn btn-sm btn-info">确认到货</button>
						                   <button type="button" class="btn btn-sm btn-info dropdown-toggle" data-toggle="dropdown">
						                    <span class="caret"></span>
						                  </button>
						                  <ul class="dropdown-menu btn-info zf-dropdown-width" role="menu">
											 <li><a href="${ctx}/lgt/dp/dispatchMission/detailed?id=${dispatchOrder.dispatchMission.id}" style="color:#fff">详情</a></li>
							              </ul>
						                </div>
									</td>
						         </shiro:hasPermission>
			...
			width="550" isOffice="false" isMulti="false" title="请选择下一步质检人"
...
<script>
	  $(function () {
	    ...
	    $("#selectQCUser").on("click", function() {
			selectUserinit({
				"selectCallBack" : function(list) {
					var selUserId = list[0].id;
					var selUserName = list[0].text;
					var dispatchOrderId = $("#dispatchOrderId").val();
					var dispatchMissionId = $("#dispatchMissionId").val();
					var dispatchOrder = {"id":dispatchOrderId, "dispatchMission.id":dispatchMissionId, "qcUser.id":selUserId, "qcUser.name":selUserName};
					ZF.ajaxQuery(true, "${ctx}/lgt/dp/dispatchOrder/saveCallIn",dispatchOrder , function(data) {
						if(data == "true") {
							$("#loading").hide();
							history.go(0);
							ZF.showTip("确认收货操作成功！", "info");
						} else {
							ZF.showTip("确认收货操作失败0_0！", "warning");
						}
					});
				}
			})
