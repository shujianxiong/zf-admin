...
<head>
	...
	<script type="text/javascript">
		...
		var abc=${fns:toJson(dispatchMission)};
	</script>
</head>
<body>
	...
	<div class="content-wrapper sub-content-wrapper">
		<section class="content-header content-header-menu">
			<h1>
				<small class="menu-active"><i class="fa fa-repeat"></i><a href="${ctx}/lgt/dp/dispatchOrder/callOutList">执行调货出库任务列表</a></small>
			</h1>
		</section>
		...
		<section class="content">
			<div class="box box-info">
			    ...
			    <form:form id="searchForm" modelAttribute="dispatchOrder" action="${ctx}/lgt/dp/dispatchOrder/callOutList" method="post" class="form-horizontal">
							...
							<tr data-index="${status.index }">
								...
								<shiro:hasPermission name="lgt:dp:dispatchOrder:edit">
    								<td>
										<div class="btn-group">
											<c:choose>
												<c:when test="${dispatchOrder.orderStatus == 1 }"><!-- 1表示为待派送状态才可以接收调货单 -->
													<button type="button" class="btn btn-sm btn-info" onclick="window.location.href='${ctx}/lgt/dp/dispatchOrder/receiveMission?id=${dispatchOrder.id}&dispatchOrder.dispatchMission.id=${dispatchOrder.dispatchMission.id}'">接收</button>
									                <button type="button" class="btn btn-sm btn-info dropdown-toggle" data-toggle="dropdown">
									                    <span class="caret"></span>
									                </button>
									                <ul class="dropdown-menu btn-info zf-dropdown-width" role="menu">
										    			<li><a href="${ctx}/lgt/dp/dispatchMission/detailed?id=${dispatchOrder.dispatchMission.id}&dispatchOrder.id=${dispatchOrder.id }" style="color:#fff">详情</a></li>
									                </ul>
												</c:when>
												<c:when test="${dispatchOrder.orderStatus == 2 }"><!-- 2已接收，执行调货单信息 -->  
													<button type="button" class="btn btn-sm btn-info" onclick="window.location.href='${ctx}/lgt/dp/dispatchOrder/confirmInfo?id=${dispatchOrder.id}&dispatchOrder.dispatchMission.id=${dispatchOrder.dispatchMission.id}'">确认调货</button>
								                    <button type="button" class="btn btn-sm btn-info dropdown-toggle" data-toggle="dropdown">
								                    <span class="caret"></span>
								                  	</button>
									                <ul class="dropdown-menu btn-info zf-dropdown-width" role="menu">
									    				<li><a href="#" onclick="send('${dispatchOrder.dispatchMission.id}', '${dispatchOrder.id}', '${dispatchOrder.postType}')">派送</a></li>
									    				<li><a href="${ctx}/lgt/dp/dispatchMission/detailed?id=${dispatchOrder.dispatchMission.id}&dispatchOrder.id=${dispatchOrder.id }" style="color:#fff">详情</a></li>
									                </ul>
												</c:when>
												<c:when test="${dispatchOrder.orderStatus == 3}"><!-- 待派送 -->
													<button type="button" class="btn btn-sm btn-info" onclick="send('${dispatchOrder.dispatchMission.id}', '${dispatchOrder.id}', '${dispatchOrder.postType }')">派送</button>
													<button type="button" class="btn btn-sm btn-info dropdown-toggle" data-toggle="dropdown">
									                    <span class="caret"></span>
									                </button>
									                <ul class="dropdown-menu btn-info zf-dropdown-width" role="menu">
									                	<li><a href="${ctx}/lgt/dp/dispatchOrder/confirmInfo?id=${dispatchOrder.id}&dispatchOrder.dispatchMission.id=${dispatchOrder.dispatchMission.id}">修改调货信息</a></li>
									    				<li><a href="${ctx}/lgt/dp/dispatchMission/detailed?id=${dispatchOrder.dispatchMission.id}&dispatchOrder.id=${dispatchOrder.id }">详情</a></li>
									                </ul>
												</c:when>
												<c:when test="${dispatchOrder.orderStatus == 4}"><!-- 派送中 -->
													<button type="button" class="btn btn-sm btn-info" onclick="window.location.href='${ctx}/lgt/dp/dispatchMission/detailed?id=${dispatchOrder.dispatchMission.id}&dispatchOrder.id=${dispatchOrder.id }'">详情</button>
													<button type="button" class="btn btn-sm btn-info dropdown-toggle" data-toggle="dropdown">
									                    <span class="caret"></span>
									                </button>
												</c:when>
...
<script>
	  ...
	  function send(dispatchMissionId, dispatchOrderId, postType) {
		  if(postType == null || postType == '' || postType == undefined) {
			  ZF.showTip("请先完善物流信息，再派送","info");
		  	  return false;
		  }
		  window.location.href="${ctx}/lgt/dp/dispatchOrder/sendMission?id="+dispatchOrderId+"&dispatchOrder.dispatchMission.id="+dispatchMissionId;
	  }
</script>
</body>
</html>