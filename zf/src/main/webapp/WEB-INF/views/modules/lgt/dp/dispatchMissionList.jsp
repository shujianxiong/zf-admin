...
<body>
	...
	<div class="content-wrapper sub-content-wrapper">
		<section class="content-header content-header-menu">
			<h1>
				<small class="menu-active"><i class="fa fa-repeat"></i><a href="${ctx}/lgt/dp/dispatchMission/">调货任务列表</a></small>
				
	        	<shiro:hasPermission name="lgt:dp:lgtPsDispatchMission:edit"><small>|</small><small><i class="fa-form-edit"></i><a href="${ctx}/lgt/dp/dispatchMission/form">调货任务${not empty dispatchMission.id ? '修改' : '添加' }</a></small></shiro:hasPermission>
			</h1>
		</section>
		...
		<section class="content">
			<div class="box box-info">
				...
				<form:form id="searchForm" modelAttribute="dispatchMission" action="${ctx}/lgt/dp/dispatchMission/" method="post" class="form-horizontal">
		        ...
		        <div class="box-body">
		        	<div class="row">
		        		...
		        		<div class="col-md-4">
		        			<div class="form-group">
			                  <label for="missionStatus" class="col-sm-3 control-label">任务状态</label>
			                  <div class="col-sm-7">
	    			...
	    			<table id="contentTable" class="table table-bordered table-hover table-striped zf-tbody-font-size">
						...
						<c:forEach items="${page.list}" var="dispatchMission" varStatus="status">
							<tr data-index="${status.index }">
								...
								<td>
									<c:choose>
										<c:when test="${dispatchMission.missionStatus == 1 || dispatchMission.approveStatus == 1}"><!-- 任务状态为1表示该任务还没确认提交是否需要审核，2表示为已确认提交等待审核 3执行，4已完成-->
											<div class="btn-group">
								                  <button type="button" class="btn btn-sm btn-info" onclick="window.location.href='${ctx}/lgt/dp/dispatchMission/form?id=${dispatchMission.id}'">修改</button>
								                   <button type="button" class="btn btn-sm btn-info dropdown-toggle" data-toggle="dropdown">
								                    <span class="caret"></span>
								                  </button>
								                  <ul class="dropdown-menu btn-info zf-dropdown-width" role="menu">
							                    	<shiro:hasPermission name="lgt:dp:lgtPsDispatchMission:edit">
													<li><a href="${ctx}/lgt/dp/dispatchMission/release?id=${dispatchMission.id}&missionCode=${dispatchMission.missionCode}" style="color:#fff">发布</a></li>
													</shiro:hasPermission>
							                  	  </ul>
						              		</div>
										</c:when>
									
					                  	<c:when test="${dispatchMission.approveStatus == 2 && dispatchMission.missionStatus == 1 }">
						                  	<div class="btn-group">
							                   <button type="button" class="btn btn-sm btn-info" onclick="window.location.href='${ctx}/lgt/dp/dispatchMission/release?id=${dispatchMission.id}&missionCode=${dispatchMission.missionCode}'">发布</button>
							              	    <button type="button" class="btn btn-sm btn-info dropdown-toggle" data-toggle="dropdown">
								                    <span class="caret"></span>
								                  </button>
								                  <ul class="dropdown-menu btn-info zf-dropdown-width" role="menu">
							                    	<shiro:hasPermission name="lgt:dp:lgtPsDispatchMission:edit">
													<li><a href="${ctx}/lgt/dp/dispatchMission/detailedAll?id=${dispatchMission.id }">详情</a></li>
													</shiro:hasPermission>
							                  	  </ul>
							              	</div>
					                    </c:when>
