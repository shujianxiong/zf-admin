...
<div class="content-wrapper sub-content-wrapper">
    <section class="content-header content-header-menu">
        <h1>
            <small class="menu-active"><i class="fa fa-repeat"></i><a href="${ctx}/crm/ns/notify/">消息列表</a></small>
            <shiro:hasPermission name="crm:ns:notify:edit">
	            <small>|</small>
                <small>
                    <i class="fa-form-edit"></i><a href="${ctx}/crm/ns/notify/form">消息添加</a>
                </small>
            </shiro:hasPermission>
        </h1>
    </section>
    ...
    <section class="content">
        <div class="box box-info">
            ...
            <form:form id="searchForm" modelAttribute="notify" action="${ctx}/crm/ns/notify/" method="post" class="form-horizontal">
                ...
                <div class="box-body">
                    <div class="row">
                            ...
                            <div  class="form-group">
                                <label for="name" class="col-sm-3 control-label">编码/标题</label>
                                <div class="col-sm-7">  
                                    <form:input path="searchParameter.keyWord" class="form-control" placeholder="请输入消息编码或标题查询"/>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                        ...
                        <tbody>
                            <c:forEach items="${page.list}" var="notify" varStatus="status">
                                <tr data-index="${status.index }">
									...
									<td>
                                        ${fns:getDictLabel(notify.remindTargetType,'crm_ns_notify_remindTargetType','') }
									</td>
									<td>
                                        ${fns:getDictLabel(notify.remindTargetAction,'crm_ns_notify_remindTargetAction','') }
									</td>
									<td>
										${notify.messageCode}
									</td>
                                    ...
                                    <shiro:hasPermission name="crm:ns:notify:edit"><td>
                                        <div class="btn-group zf-tableButton">
                                            ...
                                            <ul class="dropdown-menu btn-info zf-dropdown-width" role="menu">
                                            	<c:if test="${notify.status ne '3' }">
	                                                <li><a href="${ctx}/crm/ns/notify/form?id=${notify.id}" target="">修改</a></li>
                                            	</c:if>
                                                <c:choose>
                                                	<c:when test="${notify.status eq '1' }">
                                                		<li><a href="${ctx}/crm/ns/notify/delete?id=${notify.id}" style="color: #fff" onclick="return ZF.delRow('确定要删除该消息吗？',this.href)">删除</a>
	   													<li><a href="#this" onclick="publishNotify('${ctx}/crm/ns/notify/publish','${notify.id}','确定要提交该消息吗？')">提交</a></li>
                                                	</c:when>
                                                	<c:when test="${notify.status eq '2'}">
	   													<li><a href="#this" onclick="publishNotify('${ctx}/crm/ns/notify/publish','${notify.id}','确定要发布该消息吗？')">发布</a></li>
                                                	</c:when>
                                                	<c:when test="${notify.status eq '3'}">
	   													<li><a href="#this" onclick="publishNotify('${ctx}/crm/ns/notify/publish','${notify.id}','确定要撤销该消息吗？')">撤销</a></li>
                                                	</c:when>
                                                </c:choose>
                                            </ul>
                                        </div>
