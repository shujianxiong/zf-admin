...
<div id="${id }treeInput" class="form-group">
  	...
  	<div class="col-sm-${inputWidth }">
  		<div class="input-group">
  			<form:hidden id="${id}Id" path="${name}" />
  			<input type="hidden" id="${id}Type" />
  			<form:input id="${id}Name" path="${labelName }" data-verify="true" readOnly="true" class="form-control zf-input-readonly" placeholder="${tip }"/>
	...
	ZF.ajaxQuery(false,"${url}",postData,function(data){
		...
		for(var i=0;i<data.length;i++){
			if(data[i].pId==""||data[i].pId=="0"){
				var node={id:data[i].id,text:data[i].name,type:data[i].type,nodes:${id}getNodes(data[i].id,data)}
				treeData.push(node);
			}
		}
		var options = {
