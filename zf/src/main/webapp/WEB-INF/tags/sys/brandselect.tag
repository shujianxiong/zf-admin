...
<script type="text/javascript">
	...
	$("#${id}Button, #${id}Name").click(function(){
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/idx/bs/brand/select", "选择${title}", 880, 560, {
				...
				if (v=="ok"){
					for(var i=0;i<list.length;i++){
						if($(list[i]).is(":checked")){
							var id=$(list[i]).val();
							var brandIdInput=$(table).find("input[id='brandId"+id+"']");
							var brandIdName=$(table).find("input[id='brandName"+id+"']");
							$("#${id}Id").val($(brandIdInput[0]).val());
							$("#${id}Name").val($(brandIdName[0]).val());
							return;
						}
					}
				}
