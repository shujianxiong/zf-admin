...
<%@ attribute name="inputName" type="java.lang.String" required="false" description="输入框对应的文件名称属性"%>
<%@ attribute name="inputValue" type="java.lang.String" required="false" description="输入框对应的文件名称值"%>
...
<ol id="${input}Preview"  class="nav nav-tabs"></ol><c:if test="${!readonly}"><input type="hidden" id="fileNames" value="${inputValue }" data-type="${type}"/><a href="javascript:" onclick="${input}FinderOpen();" class="btn">${selectMultiple?'添加':'选择'}</a>&nbsp;<a href="javascript:" onclick="${input}DelAll();" class="btn">清除</a></c:if>
<script type="text/javascript">
	...
	function ${input}SelectAction(fileUrl, data, allFiles){
		var url="", fileNames="", files=ckfinderAPI.getSelectedFiles();
		...
		$("#fileNames").val($("#fileNames").val()+($("#fileNames").val(fileNames)==""?fileNames:"|"+fileNames));
	...
	function ${input}Preview(){
		var li, urls = $("#${input}").val().split("|");
 		var fileNames = $("#fileNames").val().split("|");
  		var uploadFileType = $("#fileNames").attr("data-type");
		$("#${input}Preview").children().remove();
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				li = "<li class=\"zf-img-list\"><img src=\""+urls[i]+"\" url=\""+urls[i]+"\" style=\"max-width:${empty maxWidth ? 100 : maxWidth}px;max-height:${empty maxHeight ? 100 : maxHeight}px;_height:${empty maxHeight ? 100 : maxHeight}px;border:0;padding:3px;\">";//</c:if><c:if test="${type ne 'thumb' && type ne 'images'}">
				if("files" == uploadFileType)  li = "<li class=\"zf-img-list\"><a href=\""+urls[i]+"\" url=\""+urls[i]+"\" target=\"_blank\">"+fileNames[i]+"</a>"; //</c:if>
				if("files" != uploadFileType) li = "<li class=\"zf-img-list\"><a href=\""+urls[i]+"\" url=\""+urls[i]+"\" target=\"_blank\"><img src="+urls[i]+" width=100 height=100 /></a>";
