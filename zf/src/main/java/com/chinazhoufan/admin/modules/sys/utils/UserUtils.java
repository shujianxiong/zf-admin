...
import com.chinazhoufan.admin.modules.sys.dao.OfficeDao;
...
import com.chinazhoufan.admin.modules.sys.security.SystemAuthorizingRealm.Principal;
...
public class UserUtils {
	...
	public static List<User> getUserList(){
		return userDao.findUserAll(new User());
	}
	...
	public static User getAdmin(){
		User user = (User)CacheUtils.get(USER_CACHE, USER_CACHE_ID_ + Global.ADMIN_ID);
		if (user ==  null){
			user = userDao.get(Global.ADMIN_ID);
			if (user == null){
				return null;
			}
			user.setRoleList(roleDao.findList(new Role(user)));
	 ...
	 * @return
	 */
	public static String getMenuListToJson(){
		@SuppressWarnings("unchecked")
		List<Menu> menuList = (List<Menu>)getCache(CACHE_MENU_LIST);
		if (menuList == null){
			User user = getUser();
		...
		return JsonMapper.toJsonString(menuList);
	}
	...
	/**
	 * 根据id 获取区域全称（eg:中国浙江省杭州市拱墅区XX街道等）
	 * @param id
	 * @return
	 */
	public static String getAreaFullNameById(String id){
		if(StringUtils.isBlank(id)){
			return "";
		}
		...
		for(Area areaTemp : areaList){
			if(areaTemp.getId().equals(id)){
				currentArea = areaTemp;
				break;
			}
		}
		...
		String parentIds = currentArea.getParentIds();
		String[] parentIdArr = parentIds.split(",");
		for(String parentId : parentIdArr){
			if(parentId!=null && parentId!=""){
				for(Area areaTemp2 : areaList){
					if(areaTemp2.getId().equals(parentId)){
						fullName += areaTemp2.getName();
						break;
					}
				}
			}
		}
		fullName += currentArea.getName();
		return fullName;
	}
	...
	public static List<Office> getOfficeList(){
		...
		if (officeList == null){
			User user = getUser();
			//  2017-02-13   过滤掉已停用的数据
			Office office = new Office();
			office.setUseable(Office.USEABLEOK);
			if (user.isAdmin()){
