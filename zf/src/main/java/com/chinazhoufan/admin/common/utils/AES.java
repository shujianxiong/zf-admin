...
public class AES {
	...
	public static byte[] encrypt(byte[] content, byte[] password) {
		if (content == null || password == null)
			return null;
		try {
			Cipher cipher =  Cipher.getInstance(ALGORITHM_AEPP);
	 ...
	 * @return 加密后数据
	 */
	public static byte[] encrypt(byte[] content, byte[] password, byte[] ivStr) {
		if (content == null || password == null)
			return null;
		try {
			Cipher cipher =  Cipher.getInstance(ALGORITHM_AEPP);
	 ...
	 * @return 解密后数据
	 */
	public static byte[] decrypt(byte[] content, byte[] password, byte[] ivStr) {
		if (content == null || password == null)
			return null;
		try {
			Cipher cipher = Cipher.getInstance(ALGORITHM_AEPP);
