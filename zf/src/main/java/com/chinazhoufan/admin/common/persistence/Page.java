...
//	private int pageSize = Integer.valueOf(Global.getConfig("page.pageSize")); // 页面大小，设置为“-1”表示不进行分页（分页无效）
	...
	private boolean lastPage;	// 是否是最后一页(已改成转json时保留，供页面接收数据后判断使用)
	...
	public String toString() {
		...
		sb.append("<li class=\"disabled controls\"><a href=\"javascript:\">前往第 ");
		sb.append("<input type=\"text\" value=\""+pageNo+"\" onkeypress=\"var e=window.event||this;var c=e.keyCode||e.which;if(c==13)");
		sb.append(funcName+"(this.value,"+pageSize+",'"+funcParam+"');\" onclick=\"this.select();\" style=\"height:20px;width:30px;border: none;text-align: center;\" />页 ，每页显示 ");
		sb.append("<input type=\"text\" value=\""+pageSize+"\" onkeypress=\"var e=window.event||this;var c=e.keyCode||e.which;if(c==13)");
		sb.append(funcName+"("+pageNo+",this.value,'"+funcParam+"');\" onclick=\"this.select();\" style=\"height:20px;width:30px;border: none;text-align: center;\" /> 条数据，");
