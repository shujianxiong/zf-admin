...
public class DebtWarningOneDayService extends AbstractTaskListener {
	...
	public void doTask(TaskEvent event) {
		List<ExperienceOrder> list = experienceOrderTaskService.findOneDayDebt();
		for (ExperienceOrder eo : list) {
			try {
				memberNotifyService.createByNotifyCode(Notify.MSG_DEBT_WARNING_ONE_DAY, eo.getMember(), eo.getOrderNo());
