...
<head>
	...
	function loadTree(){
		$("#tree").html("");
		var url=ctx+"/bas/fileDir/treeData/public"
		ZF.ajaxQuery(false,url,null,function(data){
			var treeData=new Array();
			var fileDirId=$("#fileDirId").val();
			var node=null;
			for(var i=0;i<data.length;i++){
				if(data[i].pId==""||data[i].pId=="0"){
					if(data[i].id==fileDirId)
						node={id:data[i].id,text:data[i].name,state:{selected: true},nodes:getNodes(data[i].id,data)}
					else
			...
			var options = {
	          ...
	          onNodeSelected: function(event, node) {
	        	  $("#fileDirId").val(node.id)
	        	  searchForm.submit();
	          }
			}
			${input}treeView=$("#tree").treeview(options);
			//创建滚动域
			ZF.createScroll("tree",$("#tree").height()<600?$("#tree").height():600)
		},function(){
	...
	//确认上传
	function upload(obj){
		if($("#fileDirId").val()==null||$("#fileDirId").val().length<=0){
			alert("请选择您要上传的目录.","warning")
			return;
		}
		...
		swfu.addPostParam("fileDirId",$("#fileDirId").val());
		swfu.startUpload();	
	}
	...
	function delFile(id){
		confirm("您确认要删除此图片吗?", "info", function() {
			...
			ZF.ajaxQuery(false,url,$.parseJSON('{"id":"'+id+'","fileDirId":"'+$("#fileDirId").val()+'"}'),function(data){
				if(data.status!=1){
...
<body>
    ...
    <section class="content">
    	<form:form id="searchForm" modelAttribute="fileLibrary" action="${ctx}/bas/fileLibrary/uploadView" method="post" >
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<input id="fileDirId" name="fileDirId" type="hidden" value="${fileDirId }" />
		<input name="selectMultiple" type="hidden" value="${selectMultiple }" />
		<div class="row">
			<!-- <div class="col-md-4">
				<div class="box box-solid" style="border:1px solid #d2d6de">
					<div id="tree" class="ztree box-body divScroll">
				...
				<div class="box box-primary">
				    <shiro:hasPermission name="bas:fileUpload">
						...
						<div class="box-header with-border">
							...
							<span id="fileNum" data-num="0" class="label label-primary" style="position:absolute;top:14px; margin-left: 4px;">您当前选择了0个文件</span>
						...
						<ul class="nav nav-tabs">
							<c:forEach items="${page.list }" var="item">
								<li class="zf-img-list" onmousemove="$(this).children()[0].style.display='block'" onmouseout="$(this).children()[0].style.display='none'">
					 				<div style="position:absolute;bottom:0;width:100%;height:30px;background:rgba(0,0,0,0.3);color:#fff;font-size:9px;display:none">
					 				...
					 				<c:choose>
					 					...
					 					<c:otherwise>
					 						<img src="${ctxStatic }/adminLte/plugins/swfupload/file.png"  width="150"  height="150" />
					 					</c:otherwise>
					 				</c:choose>
						 		</li>
							</c:forEach>
