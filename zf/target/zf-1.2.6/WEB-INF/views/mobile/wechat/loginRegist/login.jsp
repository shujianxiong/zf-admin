...
<head>
    ...
    <script src="${ctxMobile}/pages/loginRegist/login.js" type="text/javascript" ></script>
</head>
<body>
	<div class="login_bac">
		<div class="login_head">
			<span id="loginCancel" onclick="history.go(-1);">
				取消
			</span>
			<span id="loginRegist" onclick="window.location.href='${ctxWeb }/smsValidate'">
				注册
			</span>
		</div>
		<!-- form onsubmit事件增加表单验证 -->
		<form action="${ctxWeb }/doLogin" name="loginForm" method="post">
			<input type="text" id="status" value="${echos.status }" style="display:none"/>
			<div class="login_input">
				<input type="hidden" name="requestUri" value="${requestUri }" />
				<input type="text" data-notnull placeholder="请输入手机号码" id="userCode" name="userCode" value="18824678798"></input>
				<input type="password" data-notnull placeholder="请输入密码" id="passWord" name="password" value="a123456"></input>
				<p onclick="window.location.href='${ctxWeb}/smsBackPassword'">忘记密码</p>
			</div>
			<div class="login_button" id="onSubmit">登录</div>
		</form>
	</div>
