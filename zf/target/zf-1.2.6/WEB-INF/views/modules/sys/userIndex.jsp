	...
	<div id="content" class="row-fluid">
		<div id="left" class="accordion-group">
			<div class="accordion-heading">
		    	...
		    	<%-- <sys:inputtree url="${ctx}/sys/office/treeData" label="组织机构" tip="请输入组织机构" id="ztree" labelName="office.name" name="office.id"></sys:inputtree> --%>
		...
		var setting = {data:{simpleData:{enable:true,idKey:"id",pIdKey:"pId",rootPId:'0'}},
			callback:{onClick:function(event, treeId, treeNode){
					...
					if(treeNode.isParent) {
						/* $('#officeContent').attr("src","${ctx}/sys/user/list?company.id="+id+"&company.name="+encodeURIComponent(treeNode.name)); */
						$('#officeContent').attr("src","${ctx}/sys/user/list?company.id="+id);
					} else {
						var pnode = treeNode.getParentNode();
						$('#officeContent').attr("src","${ctx}/sys/user/list?office.id="+id+"&company.id="+pnode.id);
						/* $('#officeContent').attr("src","${ctx}/sys/user/list?office.id="+id+"&office.name="+encodeURIComponent(treeNode.name)+"&company.id="+pnode.id+"&company.name="+pnode.name); */
					}
				}
			}
		};
