...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/msm/mk/mystoreApply/form?id=${mystoreApply.id}">个人微店申请<shiro:hasPermission name="msm:mk:mystoreApply:edit">审核</shiro:hasPermission><shiro:lacksPermission name="msm:mk:mystoreApply:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="mystoreApply" action="${ctx}/msm/mk/mystoreApply/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="applyTime" type="text" disabled="disabled" maxlength="20" class="input-medium Wdate required"
							...
							onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});" style="width: 168px;"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">申请状态：</label>
			<div class="controls">
				<select id="status" name="status"  class="input-medium required" data-value="${ mystoreApply.status}">
					<option value=""></option>
					<c:forEach items="${fns:getDictList('msm_mk_mystore_apply_status')}" var="dict">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<sys:ckfinder input="photoUrl" type="files" uploadPath="/msm/mk/photoUrl" selectMultiple="false" readonly="true"/> --%>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="checkUser.id" value="${fns:getUserById(mystoreApply.checkUser.id).name}" readonly="true" htmlEscape="false" maxlength="64" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">审批备注：</label>
			<div class="controls">
				<form:textarea path="checkRemarks" htmlEscape="false" rows="4" maxlength="255" class="input-xxlarge "/>
			</div>
		</div>
		<div class="form-actions">
			<shiro:hasPermission name="msm:mk:mystoreApply:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
