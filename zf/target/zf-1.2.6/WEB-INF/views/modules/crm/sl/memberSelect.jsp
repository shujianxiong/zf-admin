...
<div class="content-wrapper sub-content-wrapper">
	<section class="content-header content-header-menu">
		<h1>
			<small class="menu-active"><i class="fa fa-repeat"></i><a href="${ctx}/crm/sl/smsLink/memberSelect">会员列表</a></small>
		</h1>
	</section>
	<sys:tip content="${message}"/>
	<section class="content">
		<form:form id="searchForm" modelAttribute="notifyMemberVO" action="${ctx}/crm/sl/smsLink/memberSelect" method="post" class="form-horizontal" onsubmit="return onSubmit();">
						...
						<tr>
							<td>
								<div class="zf-check-wrapper-padding">
									<input type="checkBox" name="selectName" value="${member.usercode}" data-name="${member.usercode}"/>
								</div>
							</td>
							<td>
									${member.usercode}
...
<script>
    ...
    $(function () {
        ...
        var memberusercodes = localStorage.getItem("pageNo"+$("#pageNo").val())==null||localStorage.getItem("pageNo"+$("#pageNo").val())==undefined?[]:localStorage.getItem("pageNo"+$("#pageNo").val()).split(',');
        var usercode = localStorage.getItem('usercode');
        var existMemberIds = memberusercodes;



        if(memberusercodes!=undefined&&memberusercodes.length >0){
            for(let i=0;i<memberusercodes.length;i++){
                $("[data-name]").each(function(){
                    if($(this).val() == memberusercodes[i]){
                        $(this).iCheck('check');
                    }
                });
            }
        ...
        $("[data-name]").on("ifChecked",function(){
            ...
            $("[data-name]").each(function(){
                if($(this).prop("checked")){
                    memberusercodes.push($(this).val());
                }
            });
            if(memberusercodes.length > 0){
                localStorage.setItem('usercode', $(this).attr("data-name"));
                localStorage.setItem('pageNo'+$("#pageNo").val(), memberusercodes);
// 		  		map.set('pageNo'+$("#pageNo").val(), memberIds);
                map['pageNo'+$("#pageNo").val()] = memberusercodes;
