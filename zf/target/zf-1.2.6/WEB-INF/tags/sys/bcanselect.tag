...
<script type="text/javascript">
	...
	$("#${id}Button, #${id}Name").click(function(){
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/idx/bc/bcFan/select", "选择${title}", 880, 560, {
				...
				if (v=="ok"){
					for(var i=0;i<list.length;i++){
						if($(list[i]).is(":checked")){
							var bcfanId = $(list[i]).val();
							var bcfanName = $(list[i]).attr("fan-bigcast-name");
							var weeknum = $(list[i]).attr("fan-bigcast-weeknum");
							console.log("${id}Name");
							$("#${id}Id").val(bcfanId);
							$("#${id}Name").val(bcfanName);
							return;
						}
					}
				}
