...
<script type="text/javascript">
	...
	$("#${id}Button, #${id}Name").click(function(){
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/lgt/ps/goods/select", "选择${title}", 880, 560, {
				...
				if (v=="ok"){
					for(var i=0;i<list.length;i++){
						if($(list[i]).is(":checked")){
							var id=$(list[i]).val();
							var goodsIdInput=$(table).find("input[id='goodsId"+id+"']");
							var goodsIdName=$(table).find("input[id='goodsName"+id+"']");
							$("#${id}Id").val($(goodsIdInput[0]).val());
							$("#${id}Name").val($(goodsIdName[0]).val());
							return;
						}
					}
				}
