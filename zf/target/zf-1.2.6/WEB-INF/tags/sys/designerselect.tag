...
<script type="text/javascript">
	...
	$("#${id}Button, #${id}Name").click(function(){
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/idx/bs/designer/select", "选择${title}", 880, 560, {
			ajaxData:{selectIds: ""},buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				...
				var list=$(table).find("input[name='selectName']");
				if (v=="ok"){
					for(var i=0;i<list.length;i++){
						if($(list[i]).is(":checked")){
							var id=$(list[i]).val();
							var designerIdInput=$(table).find("input[id='designerId"+id+"']");
							var designerIdName=$(table).find("input[id='designerName"+id+"']");
							$("#${id}Id").val($(designerIdInput[0]).val());
							$("#${id}Name").val($(designerIdName[0]).val());
							return;
						}
					}
				}
